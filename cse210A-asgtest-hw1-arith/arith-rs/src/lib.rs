...
/*
Code adapted from the tutorial on parser combinators in Rust:
...
pub enum Exp{
    ...
    // PowExp(&'a Exp<'a>, &'a Exp<'a>)
}

/*
Quick and dirty implementation of eval
*/
...
/*
We declare a new trait to avoid having to repetitively declare a parser type.

For that we need to declare a type for the parse result
*/
...
/*
Typical parser definition:
Fn(Input) -> Result<(Input, Output), Error>

For our basic language, it's gonna be something like:
Fn(&str) -> Result<(&str, Element), &str>

We take a string slice as input, convert it to an element
...
fn match_literal<'a>(expected: &'static str) -> impl Parser<'a, ()> {
    move |input: &'a str| match input.get(0..expected.len()) {
        Some(next) if next == expected =>  Ok((&input[expected.len()..], ())), // Slightly more complex pattern matching
        _ => Err(input),
    }
}
...
/*
Let's define the parser for ARITH
*/

/*
Numeric literal that can be:
1. unsigned number
...
/*
Matches one or more of a given string
*/
...
mod tests {
    ...
    fn pair_combinator() {
        ...
        assert_eq!(
            Err("oops"), tag_opener.parse("oops")
        );
    ...
    fn predicate_combinator() {
        ...
        assert_eq!(
            Ok(("mg", 'o')), parser.parse("omg"));
        assert_eq!(
            Err("lol"), parser.parse("lol")
        );
    }
