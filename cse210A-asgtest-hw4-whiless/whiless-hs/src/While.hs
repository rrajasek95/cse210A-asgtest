...
instance Show BExpr where
    show (BLit True) = "true"
    show (BLit False) = "false"
    show (BoolVar s) = show s
    show (BEq a b) = "(" ++ show a ++ "=" ++ show b ++ ")"
    show (BLT a b) = "(" ++ show a ++ "<" ++ show b ++ ")"
    show (BNeg a) = "¬" ++ show a
...
instance Show Stmt where
    show Skip = "skip"
    show (AAssg s a) = show s ++ " := " ++ show a
    show (Seq a b) = show a ++ "; " ++ show b
    show (IfStmt c a b) = "if " ++ show c ++ " then { " ++ show a ++ " } else { " ++ show b ++ " }"
    show (WhileStmt b c) = "while " ++ show b ++ " do { " ++ show c ++ " }"
    show (BAssg s a) = show s ++ " := " ++ show a
int :: Parser AExpr
int = do
    spaces
    n <- number
...
simpleStmt = (skip <|> assignment <|> ifElse <|> while <|> (do 
    spaces 
    reserved "{"
    s <- stmt
    spaces
...
-- Big step eval method. This will not apply to the new variant
...
smallStepEvalS :: Stmt -> Env Int Bool -> (Stmt, Env Int Bool)
smallStepEvalS s env@(Environment aVars bVars) = case s of
    Skip -> (Skip, env)
    Seq s1 s2 -> case s1 of
                    Skip -> (s2, env)
                    _ -> let (sNew, eNew) = smallStepEvalS s1 env
                         in (Seq sNew s2, eNew)
    IfStmt b s1 s2 -> if evalB b env then (s1, env) else (s2, env)
    WhileStmt b s -> 
        let boolResult = evalB b env
        in case boolResult of 
            True -> (Seq s (WhileStmt b s), env)
            False -> (Skip, env)
    AAssg (Var x) a -> let arithResult = evalA a env
        in (Skip, Environment (insert x arithResult aVars) bVars)
    BAssg (Var x) b -> let boolResult = evalB b env
        in (Skip, Environment aVars (insert x boolResult bVars))
...
interpretSSAux :: (Stmt, Env Int Bool) -> Int -> [(Stmt, Env Int Bool)]
interpretSSAux s 0 = [s]
interpretSSAux (Skip, env) _ = [(Skip, env)]
interpretSSAux (s, env) x = let (newTree, newEnv) = (smallStepEvalS s env) 
    in (newTree , newEnv): interpretSSAux (newTree, newEnv) (x - 1)

interpretSS :: String -> [(Stmt, Env Int Bool)]
interpretSS s = (\t -> interpretSSAux (t, (Environment Data.Map.empty Data.Map.empty)) 10000) $ runWhile s
...
showStep :: (Stmt, Env Int Bool) -> String
showStep (s, e) = "⇒ " ++ show s ++ ", " ++ showState e

showSteps :: [(Stmt, Env Int Bool)] -> String
showSteps stmts = intercalate "\n" $ Prelude.map showStep stmts
